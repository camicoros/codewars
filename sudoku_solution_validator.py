...
def concatenate_iterables(board):
    return list(chain(*board))


def valid_solution(board):
    def zeros_in_board(_board):
        if 0 in set(concatenate_iterables(_board)):
            return True
        return False
    ...
    def valid_rows(_board):
        for row in _board:
            if not is_unique(row):
                return False
        return True

    def valid_squares(_board):
        for i in range(0, 3):
            row1 = _board[i*3]
            row2 = _board[i*3 + 1]
            row3 = _board[i*3 + 2]
            ...
            for j in range(0, 3):
                square = []
                square.append(row1[j*3])
                square.append(row1[j*3 + 1])
                square.append(row1[j*3 + 2])
                square.append(row2[j*3])
                square.append(row2[j*3 + 1])
                square.append(row2[j*3 + 2])
                square.append(row3[j*3])
                square.append(row3[j*3 + 1])
                square.append(row3[j*3 + 2])

                if not is_unique(square):
                    return False
        return True

    def transparent_board(_board):
        return list(zip(*_board))

    if zeros_in_board(board) or\
            not valid_rows(board) or\
            not valid_rows(transparent_board(board)) or\
            not valid_squares(board):
        return False
    else:
        return True
